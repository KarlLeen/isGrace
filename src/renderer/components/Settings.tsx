...
export default function Settings() {
  const { llmSettings, saveLLMSettings, setSettingsOpen, uiLanguage, setUiLanguage, setOnboardingComplete, setOnboardingStep } = useStore();
  const { t } = useLang();

  const [provider, setProvider] = useState<LLMProvider>(llmSettings.provider ?? 'openrouter');
  const [apiKey, setApiKey]     = useState(llmSettings.apiKey);
  const [model, setModel]       = useState(llmSettings.model);
  const [temp, setTemp]         = useState(llmSettings.temperature);
