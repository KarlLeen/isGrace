...
export default function ResourcePanel() {
  const {
    uploadedMaterials, cheatsheets, studyGuide, chapters, tests,
    removeMaterial, addMaterial, setActivePanel, setActiveTest,
    setLayoutMode,
  } = useStore();
  const { t } = useLang();
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto" style={{ backgroundColor: '#FEFEFE' }}>
        ...
        {uploadedMaterials.length === 0 ? (
          <button
            ...
            style={{
              ...
              border: '1.5px dashed #E0DBD4', backgroundColor: 'transparent',
              cursor: 'pointer', fontSize: 12, color: '#C0BAB2',
      ...
      {/* Generated content */}
      {(cheatsheets.length > 0 || studyGuide || tests.length > 0 || chapters.length > 0) && (
        <>
          ...
          <section style={{ padding: '12px 14px 16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {chapters.length > 0 && !tests.length && (
                <ActionRow
                  icon={<TestIcon />}
                  label={t.resTest(chapters.length)}
